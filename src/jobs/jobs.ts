...
import { getBookingAISummaryQueue } from "./bookingAISummary.queue";
...
async function startQueue() {
    ...
    await startBookingAISummaryQueue();
}
...
async function startBookingAISummaryQueue() {
    // Process jobs from the queue
    const bookingAISummaryQueue = await getBookingAISummaryQueue();

    bookingAISummaryQueue.process("booking-ai-summary-queue", async (job) => {
        const bookingSummary = job.data;

        try {
            const response = await generateBookingSummary(bookingSummary);
            console.log("LOG: ~ bookingAISummaryQueue ~ response:", response);
            return response;
        } catch (err: unknown) {
            // TODO  ADD LOGGING
            console.error(`Retrying job ${job.id} due to error:`, err);
    ...
    // Event listener for completed jobs
    bookingAISummaryQueue.on("failed", (job, err) => {
        console.error(
            `Job ID ${job.id} failed on attempt ${job.attemptsMade} of ${job.opts.attempts}. Error:`,
            err,
        );
    ...
    bookingAISummaryQueue.on("completed", (job, result) => {
        console.log(
            `✅ Job ID ${job.id} succeeded after ${job.attemptsMade} attempt(s).`,
        );
    });
